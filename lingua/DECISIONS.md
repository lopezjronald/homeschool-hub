...
- **SPIKE-03** Frequency-band leveling validated vs Fluency Matters L1 novellas (should score "easy"). — _MECHANISM VALIDATED (2026-07-24); calibration pending 2 decisions. Harness: `lingua/spikes/spike03_leveling/`._
  - **Algorithm works.** Coverage-first scorer (accent-aware tokenize → out-of-band % → L1..L8 ladder) cleanly separates difficulty on legally-clean texts: hand-authored top-frequency text → **L1** (0% out-of-band); lexically-rich text → **L8** (75%); public-domain Don Quijote opening → **L7** (51.5%). The scorer is the reusable M1 engine core; swapping the corpus is a one-line data change.
  - **Not yet calibrated**, blocked on 2 choices: (1) the real frequency corpus — `wordfreq` (pip; MIT; bundles Spanish incl. SUBTLEX-derived data) vs SUBTLEX-ESP (OSF download) vs provided; (2) legally-sourced graded L2 texts — Fluency Matters text is **copyrighted (D-47)**, so validate against owned copies (local fair-use analysis) or public-domain graded readers. Both involve a download/install and/or copyright, so they're operator decisions.
  - Refinement for production: add lemmatization (simplemma, D-26) so conjugations collapse to lemmas (the spike matches surface forms).
