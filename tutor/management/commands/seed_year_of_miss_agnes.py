"""Seed Violet's Blackbird & Company "The Year of Miss Agnes" course (idempotent).
...
boxes.
    ...
    python manage.py seed_year_of_miss_agnes --for-user ronald
    python manage.py seed_year_of_miss_agnes --for-user ronald --dry-run
"""
...
class Command(BaseCommand):
    help = "Seed The Year of Miss Agnes (Blackbird Level 3): 4 sections + a final project."
    ...
    def handle(self, *args, **options):
        ...
        blueprint = get_blueprint("blackbird_year_of_miss_agnes")
    ...
    def _hands_on_project(self, curriculum, family):
        ...
        book = glean_handson.BOOKS["miss_agnes"]
        lesson = self._lesson(curriculum, 5, 1)
        hands_on = glean_handson.hands_on_title("miss_agnes")
        # Renaming a PROJECT must not leave the retired one beside its
        # replacement — the child would be offered both.
        glean_handson.retire_superseded(lesson, hands_on)
        return self._set(
            lesson,
            family,
            hands_on,
