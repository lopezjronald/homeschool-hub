...
| D-03 | No FK from `lingua` to host models; learner carries a plain scalar | **CHANGED** | Scalar is **`host_student_id`** → `students.Student.pk`, NOT `host_user_id` — children have no user row (tokenless `portal_key`). Rule itself kept: cross-app FKs block extraction. **Superseded in part by D-70** — a learner may now be an adult, keyed by `host_user_id`. The no-FK rule is untouched. |
...
| D-70 | A `Learner` may be an ADULT, keyed by `host_user_id` instead of `host_student_id` (exactly one, enforced by a CheckConstraint) | **LOCKED** | Supersedes D-03's clause that the scalar is never `host_user_id`. The RULE (no cross-app ForeignKey) is untouched — this is still a plain int resolved through the adapter. The parent deliberately has NO `Student` row, which is what keeps him out of the kids' roster, portal tokens and charter records for free: all of those resolve through `directory.family_children()`, which only returns Students. Note D-36 predicted TEEN/ADULT would be "a data change later"; it was a schema change. |
| D-71 | Adult AI conversation is an OUTBOUND LINK ONLY; no conversation code in this codebase | **LOCKED** | The research is unambiguous that generative voice tutors are inappropriate for these children, and every such product's own terms exclude under-18s. Because the app is not the thing running the conversation, there is nothing for a child to reach even if the login gate failed — and D-52 (no child data to AI) / D-54 (AI disclosure) never attach. Configured via `LINGUA["ADULT_CONVERSATION_URL"]`; set it empty and the section disappears. |
| D-72 | `TravelPhrase` is a separate table from `ClassroomPhrase`, with NO `age_band` field | **LOCKED** | `classroom_phrases_with_audio` renders every active row on the kids' session page, so extra categories there would leak pharmacy and emergency phrases onto a nine-year-old's screen. The omitted `age_band` is deliberate: there is no way to make this content child-facing, rather than a flag someone could flip. |
...
- **SPIKE-03** Frequency-band leveling validated. — **COMPLETE (2026-07-24). Corpus: wordfreq (chosen over the OSF SUBTLEX download); calibration: public-domain texts.** Harness: `lingua/spikes/spike03_leveling/`.
  - **Validated with the real corpus** (`wordfreq.zipf_frequency(w, "es")`). On a legally-clean public-domain gradient the coverage-first scorer orders difficulty correctly: simple story **L1** (0% out-of-band) < *Los pollitos* rhyme **L2** (7.5%) < Samaniego fable **L2** (9.7%) < Don Quijote opening **L3** (15.2%). `RARE_ZIPF=3.5`.
  - **Two findings that shape M1/M3:** (1) **lemmatization (D-26) is required** — at a higher cutoff, conjugations (come/duerme/juegan) were flagged false-rare while their lemmas (comer/dormir/jugar) are common. (2) **count-only out-of-band % saturates** at a high cutoff (every literary text ~32% at 4.5 → Don Quijote stopped separating from a nursery rhyme), so the **M3** engine must use **learner-known-words coverage (true i+1)** + a richer metric, not corpus frequency alone. For **v1** the frequency-band filter is a **soft signal** (flag out-of-band words) atop hand-assigned levels — validated as fit for exactly that.
  - **wordfreq** (MIT, 3.1.1) is the chosen frequency corpus; pip-installed for the spike, pin it in `requirements.txt` when the M1 leveling code consumes it.
